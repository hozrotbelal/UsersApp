...
class CustomSwitch extends StatefulWidget {
  ...
  final bool value;
  final double height;
  final double isActiveIconWiseWidth;
  ...
  CustomSwitch({
    ...
    _activeTextWidget = Text(
      activeText,
      style: TextStyle(color: activeTextColor, fontWeight: FontWeight.bold, fontSize: fontSize),
    );

    _inactiveTextWidget = Text(
      inactiveText,
      style: TextStyle(color: inactiveTextColor, fontWeight: FontWeight.bold, fontSize: fontSize),
    );

    // Get size required for text. Max size of the above
    _spaceRequiredForText = max(_textSize(_activeTextWidget).width, _textSize(_inactiveTextWidget).width);
  }
  ...
  Size _textSize(Text text) {
    ...
    final TextPainter textPainter = TextPainter(text: TextSpan(text: text.data, style: text.style), maxLines: 1, textDirection: TextDirection.ltr)..layout(minWidth: 0, maxWidth: double.infinity);
        ...
        return GestureDetector(
            ...
            child: Tooltip(
              ...
              child: Container(
                ...
                width: widget.activeIcon != null ? widget.isActiveIconWiseWidth : widget._spaceRequiredForText + widget.height + 12,
                height: widget.height,
                margin: const EdgeInsets.only(top: 4.0, bottom: 4.0, right: 4.0, left: 4.0),
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(50.0), color: widget.value ? widget.activeColor : widget.inactiveColor),
                child: Padding(
                    ...
                    children: [
                      widget.value
                          ? Padding(
                              ...
                              child: widget.activeIcon ?? widget._activeTextWidget,
                            )
                          : Container(),
                      Align(
                        ...
                        child: Container(
                            ...
                            decoration: BoxDecoration(shape: BoxShape.circle, color: widget.value ? widget.activeThumbColor : widget.inactiveThumbColor),
                            child: widget.value ? widget.activeThumbIcon ?? Container() : widget.inactiveThumbIcon ?? Container()),
                      ),
                      !widget.value
                          ? Padding(
                              padding: const EdgeInsets.only(left: 6, right: 0.0),
                              child: widget.inactiveIcon ?? widget._inactiveTextWidget,
                            )
                          : Container(),
                    ],
                  ),
